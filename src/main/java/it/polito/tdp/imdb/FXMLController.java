...
public class FXMLController {
    ...
    void doCreaGrafo(ActionEvent event) {
    	...
    	boxRegista.getItems().removeAll(boxRegista.getItems());
    ...
    void doRegistiAdiacenti(ActionEvent event) {
    	...
    	List<Adiacenza> vicini=model.getRegistiAdiacenti(d);
    ...
    void doRicorsione(ActionEvent event) {
    	...
    	String s_c=txtAttoriCondivisi.getText();
    	Integer c=0;
    	try {
    		c=Integer.parseInt(s_c);
    	} catch(NumberFormatException e) {
    		txtResult.setText("Selezionare il numero minimo di attori condivisi");
    		return;
    	}
    	List<Director> result=model.trovaPercorso(d, c);
    	//txtResult.appendText("REGISTI ADIACENTI A "+d+"\n");
    	for(Director di:result) {
